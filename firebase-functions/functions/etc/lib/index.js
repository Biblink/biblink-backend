...
exports.annotationSubreply = functions.firestore.document('studies/{studyId}/annotations/{annotationName}/{annotationType}/{annotationId}/replies/{replyId}/subreplies/{subreplyId}').onWrite((event) => {
    if (!event.data.exists) {
        return null;
    }
    const data = event.data.data();
